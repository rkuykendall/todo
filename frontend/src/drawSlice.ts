...
const drawSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(fetchDraws.fulfilled, (state, action: PayloadAction<TicketDraw[]>) => {
      ...
      .addCase(patchDraw.fulfilled, (state, action: PayloadAction<TicketDraw>) => {
        const index = state.draws.findIndex(d => d.id === action.payload.id);
        if (index !== -1) {
          state.draws[index] = action.payload;
        }
      })
      .addCase(createDraws.fulfilled, (state, action: PayloadAction<TicketDraw[]>) => {
        state.draws = action.payload;
      });
  },
});
